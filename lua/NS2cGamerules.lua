...
class 'NS2cGamerules' (Gamerules)

NS2cGamerules.kMapName = "ns2c_gamerules"
...
function NS2cGamerules:ChooseTechPoint(techPoints, teamNumber)
...
if Server then
    ...
    NS2cGamerules.kMarineStartSound = PrecacheAsset("sound/ns2c.fev/ns2c/ui/marine_gamestart")
    NS2cGamerules.kAlienStartSound = PrecacheAsset("sound/ns2c.fev/ns2c/ui/alien_gamestart")
    NS2cGamerules.kCountdownSound = PrecacheAsset("sound/NS2.fev/common/countdown")
    ...
    function NS2cGamerules:BuildTeam(teamType)
    ...
    function NS2cGamerules:SetGameState(state)
    ...
    function NS2cGamerules:GetGameTimeChanged()
        return self.timeSinceGameStateChanged
    end

    function NS2cGamerules:GetGameState()
        return self.gameState
    end

    function NS2cGamerules:OnCreate()
    ...
    function NS2cGamerules:OnDestroy()
    ...
    function NS2cGamerules:GetFriendlyFire()
        return false
    end
    
    // All damage is routed through here.
    function NS2cGamerules:CanEntityDoDamageTo(attacker, target)
        return CanEntityDoDamageTo(attacker, target, Shared.GetCheatsEnabled(), Shared.GetDevMode(), self:GetFriendlyFire())
    end
    
    function NS2cGamerules:OnClientDisconnect(client)
    ...
    function NS2cGamerules:OnEntityCreate(entity)
    ...
    function NS2cGamerules:OnEntityDestroy(entity)
    ...
    function NS2cGamerules:OnEntityChange(oldId, newId)

        PROFILE("NS2cGamerules:OnEntityChange")
    ...
    function NS2cGamerules:OnEntityKilled(targetEntity, attacker, doer, point, direction)
    ...
    function NS2cGamerules:LogoutCommanders()
     ...
     */
    function NS2cGamerules:ResetGame()
        ...
        for index, entity in ientitylist(Shared.GetEntitiesWithClassname("Entity")) do
            ...
            // Don't reset/delete NS2cGamerules or TeamInfo.
            // NOTE!!!
            ...
            // at the start of the next game, including the NS2cGamerules. This is how a map transition would have to work anyway.
            ...
            local shieldTypes = { "GameInfo", "MapBlip", "NS2cGamerules" }
    ...
    function NS2cGamerules:GetTeam1()
        return self.team1
    end
    
    function NS2cGamerules:GetTeam2()
        return self.team2
    end
    
    function NS2cGamerules:GetWorldTeam()
        return self.worldTeam
    end
    
    function NS2cGamerules:GetSpectatorTeam()
        return self.spectatorTeam
    end
    
    function NS2cGamerules:GetTeams()
     ...
     */
    function NS2cGamerules:SetHiveTechIdChosen(hive, techId)
    ...
    function NS2cGamerules:UpdateScores()
    ...
    function NS2cGamerules:UpdatePings()
    ...
    function NS2cGamerules:UpdateHealth()
    ...
    function NS2cGamerules:UpdateTechPoints()
    ...
    function NS2cGamerules:CastVoteByPlayer(voteTechId, player)
    ...
    function NS2cGamerules:OnMapPostLoad()
    ...
    function NS2cGamerules:UpdateToReadyRoom()
    ...
    function NS2cGamerules:UpdateMapCycle()
    ...
    function NS2cGamerules:OnUpdate(timePassed)
    
        PROFILE("NS2cGamerules:OnUpdate")
     ...
     */
    function NS2cGamerules:EndGame(winningTeam)
    ...
    function NS2cGamerules:OnTournamentModeEnabled()
        self.tournamentMode = true
        self.sponitor.tournamentMode = true
    end
    
    function NS2cGamerules:OnTournamentModeDisabled()
        self.tournamentMode = false
        self.sponitor.tournamentMode = false
    end
    
    function NS2cGamerules:DrawGame()
    ...
    function NS2cGamerules:GetTeam(teamNum)
    ...
    function NS2cGamerules:GetRandomTeamNumber()
    ...
    function NS2cGamerules:GetCanJoinTeamNumber(teamNumber)
    ...
    function NS2cGamerules:GetCanSpawnImmediately()
     ...
     */
    function NS2cGamerules:JoinTeam(player, newTeamNumber, force)
    ...
    function NS2cGamerules:SetGameStarted()
    ...
    function NS2cGamerules:SetPreventGameEnd(state)
        self.preventGameEnd = state
    end
    
    function NS2cGamerules:SetTeamsReady(ready)
    ...
    function NS2cGamerules:SetPaused()    
    end
    
    function NS2cGamerules:DisablePause()
    end
    
    function NS2cGamerules:CheckGameStart()
        ...
        if self:GetGameState() == kGameState.NotStarted or self:GetGameState() == kGameState.PreGame then
            ...
            if (team1Players > 0 and team2Players > 0) or(Shared.GetCheatsEnabled() and (team1Players > 0 or team2Players > 0)) and (not self.tournamentMode or self.teamsReady) then
    ...
    function NS2cGamerules:CheckGameEnd()
    ...
    function NS2cGamerules:GetCountingDown()
    ...
    function NS2cGamerules:GetPregameLength()
    ...
    function NS2cGamerules:UpdatePregame(timePassed)
        ...
        elseif self:GetGameState() == kGameState.Countdown then
            ...
            if self.lastCountdownPlayed ~= countDownSeconds and (countDownSeconds < 4) then
                ...
                self.worldTeam:PlayPrivateTeamSound(NS2cGamerules.kCountdownSound)
                self.team1:PlayPrivateTeamSound(NS2cGamerules.kCountdownSound)
                self.team2:PlayPrivateTeamSound(NS2cGamerules.kCountdownSound)
                self.spectatorTeam:PlayPrivateTeamSound(NS2cGamerules.kCountdownSound)
            ...
            if self.countdownTime <= 0 then
                ...
                self.team1:PlayPrivateTeamSound(ConditionalValue(self.team1:GetTeamType() == kAlienTeamType, NS2cGamerules.kAlienStartSound, NS2cGamerules.kMarineStartSound))
                self.team2:PlayPrivateTeamSound(ConditionalValue(self.team2:GetTeamType() == kAlienTeamType, NS2cGamerules.kAlienStartSound, NS2cGamerules.kMarineStartSound))
    ...
    function NS2cGamerules:GetLosingTeam()
        return self.losingTeam
    end

    function NS2cGamerules:GetAllTech()
        return self.allTech
    end

    function NS2cGamerules:SetAllTech(state)
    ...
    function NS2cGamerules:GetAutobuild()
        return self.autobuild
    end

    function NS2cGamerules:SetAutobuild(state)
        self.autobuild = state
    end

    function NS2cGamerules:SetOrderSelf(state)
        self.orderSelf = state
    end

    function NS2cGamerules:GetOrderSelf()
        return self.orderSelf
    end

    function NS2cGamerules:GetIsPlayerFollowingTeamNumber(player, teamNumber)
    ...
    function NS2cGamerules:GetCanPlayerHearPlayer(listenerPlayer, speakerPlayer)
    ...
    function NS2cGamerules:RespawnPlayer(player)
    ...
    function NS2cGamerules:BanPlayerFromCommand(playerId)
        ASSERT(type(playerId) == "number")
        table.insertunique(self.bannedPlayers, playerId)
    end

    function NS2cGamerules:GetPlayerBannedFromCommand(playerId)
        ASSERT(type(playerId) == "number")
...
function NS2cGamerules:GetGameStartTime()
    return ConditionalValue(self:GetGameStarted(), self.gameStartTime, 0)
end

function NS2cGamerules:GetGameStarted()
    return self.gameState == kGameState.Started
end

Shared.LinkClassToMap("NS2cGamerules", NS2cGamerules.kMapName, { })
