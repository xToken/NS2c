    ...
    if(player and not player:GetIsThirdPerson()) then
        ...
        if(weapon ~= nil) then
            ...
            elseif mapname == Shotgun.kMapName then
                index = 3
            elseif mapname == GrenadeLauncher.kMapName then
                index = 5
                -- All alien crosshairs are the same for now
            elseif mapname == LerkBite.kMapName or mapname == Spores.kMapName or mapname == Umbra.kMapName or mapname == Parasite.kMapName or mapname == BileBomb.kMapName then
                index = 6
...
function PlayerUI_GetShowGiveDamageIndicator()
    ...
    if player and player.GetDamageIndicatorTime and player:GetIsPlaying() then
        ...
        local opacity = math.min(timePassed / kShowGiveDamageTime, 1)
        return timePassed <= kShowGiveDamageTime, opacity * opacity
