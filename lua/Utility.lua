...
local function PredictionValuesToString(values)
    ...
    for k,v in pairs(values) do
        ...
        elseif (type(v) == "number") then
            ...
            data = data .. string.format( "%s=%.5f ", k, v ) 
        else
...
function GetClampedScreenPosition(worldPosition, buffer)
    ...
    local toPosition = GetNormalizedVector(cameraCoords.origin - worldPosition)
    local dotProduct = cameraCoords.zAxis:DotProduct(toPosition)
    
    if dotProduct < 0 then
        VectorCopy(Client.WorldToScreen(worldPosition), screenPosition) 
    else
        ...
        screenPosition.x = screenPosition.x * kMaxScreenWidth +  Client.GetScreenWidth() * .5
        screenPosition.y = screenPosition.y * kMaxScreenWidth +  Client.GetScreenHeight() * .5
    end
