...
local kTextFontName = "MicrogrammaDMedExt"
...
function GUIAlienSpectatorHUD:Initialize()
    ...
    self.spawnText:SetFontSize(kFontSize)
    ...
    self.autoSpawnText:SetFontSize(kFontSize)
...
function GUIAlienSpectatorHUD:Update(deltaTime)
    ...
    if AlienUI_GetInEgg() then
        ...
        if timeToWave == 0 then
            self.spawnText:SetText(Locale.ResolveString("WAITING_TO_SPAWN") .. string.format(" In Position %s", AlienUI_GetSpawnQueuePosition()))
        else
