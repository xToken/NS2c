...
function GetIsUnitActive(unit, debug)
    ...
    local isRecycled = HasMixin(unit, "Recycle") and (unit:GetIsRecycled() or unit:GetRecycleActive())
    ...
    return alive and isBuilt and powered and not isRecycled
...
function GetCommanderPickTarget(player, pickVec, worldCoordsSpecified, forBuild, ignoreEntities, traceThickness, traceType )

    local traceType = traceType or 'ray'
    local ogPickVec = pickVec
    ...
    while true do
        ...
        local filter = ignoreEntities == true and EntityFilterAll() or EntityFilterOne(player)
    ...
    if traceType == 'ray' and traceThickness and not trace.entity then
        local trace2 = GetCommanderPickTarget(player, ogPickVec, worldCoordsSpecified, forBuild, ignoreEntities, traceThickness, 'box' )
...
function BuildClassToGrid()
	...
	ClassToGrid["AdvancedTurretFactory"] = { 7, 5 }
