...
GUIHiveStatus.kUpdateRate = 1 --32Hz  --0.04 --25Hz UI update-throttle
...
GUIHiveStatus.kContainerFrameTexOffsets[6] = Vector( 0, 500, 0 ) -- Create new one for Whiphive
...
GUIHiveStatus.kHiveTypeIconTexOffsets[4] = Vector( 195, 60, 0 )
...
function GUIHiveStatus:SetHiveTypeIcon( slotIdx, typeFlag )
    ...
    assert( typeFlag >= 0 and typeFlag < 7 )
...
function GUIHiveStatus:UpdateHiveIconDisplay( slotIdx, typeFlag, builtScalar, healthScalar, maxHealth, inCombat )
    ...
    assert( slotIdx > 0 and slotIdx <= GUIHiveStatus.kMaxStatusSlots )
    assert( type(typeFlag) == "number" )
    assert( typeFlag >= 0 and typeFlag < 7 ) --TODO Need enum/range global(s)
...
function GUIHiveStatus:Update( deltaTime ) 
    ...
    for locIdx = 1, #self.validLocations do
        ...
        if locationId then
            ...
            local slotData = AlienUI_GetHiveStatusForLocation(locationId)
